// src/pages/UserManagementPage.jsx (ฉบับสมบูรณ์ที่แก้ไข Error)
...
const UserManagementPage = ({ isEditMode = false }) => {
    ...
    const groupedPermissions = useMemo(() => {
        if (!allPermissions || allPermissions.length === 0) {
            return {}; // <-- เพิ่มการตรวจสอบ: ถ้าไม่มีข้อมูล ให้ return object ว่าง
        }
                                ...
                                <tbody>
                                    {userLogs.map((log, index) => (
                                        <tr key={index}>
                                            ...
                                            <td>{log.actor}</td>
                                            <td>{log.action}</td>
                                            <td style={{ whiteSpace: 'pre-wrap', fontSize: '0.85em' }}>{log.details}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                        ) : (
                            <p>No change history found for this user.</p>
                        )}
                    </fieldset>
                )}
